        ...
        while (true) // 如果是网络超时则多试几次
        {
            try {
                ...
                if (e instanceof JedisConnectionException || e instanceof SocketTimeoutException) {
                    ...
                    String redisInfo = "jedis NumActive=" + jedisSentinelPool.getNumActive() + ", NumIdle="
                            + jedisSentinelPool.getNumIdle() + ", NumWaiters="
                            + jedisSentinelPool.getNumWaiters() + ", isClosed="
                            + jedisSentinelPool.isClosed();
                    LOGGER.error("getJedis: get unknow excepiton = " + e.getMessage()
                            + " ,redis status = " + redisInfo, e);
                    break;
                }
            }
        }
