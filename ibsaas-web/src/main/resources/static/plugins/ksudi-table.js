...
/**
 * 文件类型
 * @enum {integer} FILETYPE
 */
const FILETYPE = {
    /** excel */
    EXCEL: {type: 1, name: 'EXCEL'},
    /** pdf */
    PDF: {type: 2, name: 'PDF'},
    /** image */
    IMAGE: {type: 3, name: 'IMAGE'}
};
 ...
 * @property {string} ksudiStaticPath 静态资源路径，必填
 ...
 * @property {boolean} hideFlag 是否隐藏，默认false
 ...
 * KsudiTable
 * @param $element 加载目标
 * @param options 使用{@link defaults}配置
 * @constructor
 * @author Tony
 * @version V1.2
 */
let KsudiTable = function ($element, options) {
    this.$element = $element;
    this.options = options = $.extend(true, {}, defaults, options || {});
};

KsudiTable.prototype = {
    constructor: KsudiTable,
    /**
     * 建造页面
     */
    build: function () {
        ...
        _fuseOptions(this.options);
        let options_ = this.options;
        //当前生效的配置
        let effectiveOptions = options_.currentTab;
        console.log(effectiveOptions);
        //一定要先渲染表格
        let $table = _createTableContainer(effectiveOptions);
        this.$element.html($table);
        $table.before(_createButtonContainer(effectiveOptions))
            .before(_createTabContainer(options_));//生成标签要用原来的options
        if (effectiveOptions.autoSearch) {
            _search(effectiveOptions);
        }
...
function _search(options) {
    ...
    $.get(
        ...
        function (data) {
            ...
            //渲染pages
            let $pages = _createPages(options, data);
...
function _resize(options) {
    ...
    //表格内容
    let $bodyTr = $target.find('.ksd-rt-content tr');
    $target.find('.ksd-table').css("height", tableTitleHeight + $bodyTr.length * $bodyTr.height());
    ...
    if (tableHeight >= scrollHeight) {
        ...
        $target.find('.ksd-table-operate').css('height', scrollHeight);
    }
    ...
    // 出现横向滚动条时表格宽度
    let $content = $target.find('.ksd-rt-content');
    //实际高度
    let offsetHeight = $content[0].offsetHeight;
    //可视高度
    let clientHeight = $content[0].clientHeight;
    if (offsetHeight > clientHeight) {
        $target.find('.ksd-table-operate').css('maxHeight', scrollHeight - 20).css('height', scrollHeight - 20);
    }
}
...
function _reset(options) {
    ...
    $searchForm.find("select").trigger("change");
    options.resetFunc();
}
 ...
 * @private
 */
function _createButtonContainer(options) {
    let $target = options.target;
    let $buttonContainer = $("");
    ...
    if (buttons.length > 0) {
        ...
        $.each(buttons, function (i, buttonOption) {
            ...
            if (buttonOption.methods.show() && _hasPermission(buttonOption)) {
 ...
 * @private
 */
function _createTabContainer(options) {
    let $tabContainer = $("");
    let tabs = options.tabs;
    if (tabs.length > 0) {
        let $container = $(document.createElement('div'));
        ...
        let $searchForm = options.searchForm;
        $.each(tabs, function (i, tabOption) {
            if (tabOption.methods.show() && _hasPermission(tabOption)) {
                ...
                $tab.unbind("click").bind("click", function () {
                    ...
                    //重新渲染表头
                    let $table = _createTableContainer(tabOption);
 ...
 * @private
 */
function _createTableContainer(options) {
 ...
 * @private
 */
function _createOperationDiv(options) {
    let $operationDiv = $("");
    if (options.multipleEnable || !options.multipleEnable && options.operations.length > 0) {
 ...
 * @private
 */
function _createTableContent(options) {
 ...
 * @private
 */
function _createTableHeader(options) {
    let $target = options.target;
    ...
    if (fields.length > 0) {
        ...
        $.each(fields, function (i, field) {
            field = $.extend(true, {}, defaultField, field);
            if (!field.primaryKey && !field.hideFlag) {
 ...
 * @private
 */
function _createTableBody(options, data) {
    let $target = options.target;
    //操作列标题
    if (options.multipleEnable || !options.multipleEnable && options.operations.length > 0) {
    ...
    if (fields.length > 0 && data && data.data) {
        $.each(data.data, function (i, item) {
            ...
            if (options.multipleEnable || !options.multipleEnable && options.operations.length > 0) {
                if (options.multipleEnable) {
                    ...
                    $.each(options.operations, function (i, operation) {
                        ...
                        if (operationOption.methods.show(item) && _hasPermission(operationOption)) {
            ...
            $.each(fields, function (j, field) {
                //主要业务主键列，不渲染页面元素
                let value = _getFieldValue(item, field.name);
                value = _format(field.type, value, field.formatFunc);
                if (field.primaryKey) {
                    $tr.data("value", value);
                    $optr.data("value", value);
                    $optr.find("input").val(value);
                } else if (!field.hideFlag) {
 ...
 * @private
 */
function _getFieldValue(data, fieldName) {
 ...
 * @private
 */
function _createTableFooter(options) {
    let $tableFooter = $(document.createElement("div")).addClass("ksd-table-footer").addClass("f-cb");
    //导出
    $tableFooter.append(_createExportCombobox(options));
    if (options.paginationEnable) {
        ...
        $pagination.append(_createTotalPageInfo(options));
        $pagination.append(_createPages(options));
    }
    return $tableFooter;
}
 ...
 * @private
 */
function _createExportCombobox(options) {
    let $exportCombobox = $("");
    if (options.exportEnable) {
        ...
        $.each(exportOptions.fileType, function (i, type) {
            ...
            $exportType.click(function () {
                // $exportType.addClass("active").siblings().removeClass("active");
                _export(options, exportOptions, type);
            })
        });
    }
 ...
 * @private
 */
function _createTotalPageInfo(options) {
    let $target = options.target;
    let $searchForm = options.searchForm;
 ...
 * @private
 */
function _createPages(options, data) {
    let $target = options.target;
    let $searchForm = options.searchForm;
    let ksudiStaticPath = options.ksudiStaticPath;
    let $ksdPage = $(document.createElement("div")).addClass("ksd-page");
    //首页
    let $first = $(document.createElement("a")).attr("val", 1)
        .html($(document.createElement("img")).attr("src", ksudiStaticPath + "/ksudi/images/icon-first.png"));
    ...
    if (data && data.pagination) {
        ...
        let $prev = $(document.createElement("a")).attr("val", pagination[PAGINATION.PREVIOUS_PAGE] ? pageNum - 1 : 1)
            .html($(document.createElement("img")).attr("src", ksudiStaticPath + "/ksudi/images/icon-prev.png"))
            .unbind('click').bind('click', function () {
                $searchForm.find(".pageNum").val($(this).attr("val"));
                _search(options)
            });
        ...
        let $next = $(document.createElement("a")).attr("val", pagination[PAGINATION.NEXT_PAGE] ? pageNum + 1 : pageNum)
            .html($(document.createElement("img")).attr("src", ksudiStaticPath + "/ksudi/images/icon-next.png"))
            .unbind('click').bind('click', function () {
                $searchForm.find(".pageNum").val($(this).attr("val"));
                _search(options)
            });
        ...
        let $last = $(document.createElement("a")).attr("val", pagination[PAGINATION.PAGE_COUNT])
            .html($(document.createElement("img")).attr("src", ksudiStaticPath + "/ksudi/images/icon-last.png"))
            .unbind('click').bind('click', function () {
                $searchForm.find(".pageNum").val($(this).attr("val"));
                _search(options)
            });
 ...
 * @private
 */
function _resetPageInfo(options) {
 ...
 * @private
 */
function _format(type, value, callback) {
    if (FORMAT.DATE === type) {
        return _dateFormat(value, 'yyyy-MM-dd');
    } else if (FORMAT.DATETIME === type) {
        return _dateFormat(value, 'yyyy-MM-dd hh:mm:ss');
    } else if (FORMAT.MONEY === type) {
        return _moneyFormat(value);
    } else if (FORMAT.DISTANCE === type) {
        return _distanceFormat(value);
    } else if (FORMAT.WEIGHT === type) {
        return _weightFormat(value);
 ...
 * @private
 */
function _distanceFormat(value) {
    if (value == null || value === 0 || value === "0") {
        return "0.00";
    }
    let result = "";
...
function _export(options, exportOptions, exportType) {
    //解析导出字段
...
function _hasPermission(options) {
    let psList = typeof(PERMISSION_SIGNALS) === "undefined" ? "" : PERMISSION_SIGNALS;
    if (psList === "" || psList === "[]") {
        return false;
    }
    if (options.ps === "") {
        ...
        let marchReg = new RegExp("([\\,\\[]{1}\\s?)" + options.ps.trim() + "(\\s?[\\,\\]]{1})");
